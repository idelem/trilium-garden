...
function findHtml(folder) {
    ...
    for (var i = 0; i < files.length; i++) {
        ...
        if (stat.isDirectory() && !EXCLUDE_DIRS.includes(files[i])) {
        ...
        else if (!stat.isDirectory() && isHtml(filename) && !EXCLUDE_FILES.includes(files[i])) {
            htmls.push(files[i]);
        };
    };
    return htmls;
